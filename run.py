...
for i in range(1, 31):
    ...
    if os.path.exists("stats.csv"):
        os.makedirs(destination_dir, exist_ok=True)                                      #change as needed quatumIS0.02.csv"
        shutil.copy("stats.csv", os.path.join(destination_dir, f"{schuedlerType}-{i}-006quatumIS0.02.csv"))
        shutil.copy("generalStats.csv", os.path.join(destination_dir2, f"{schuedlerType}-{i}-006quatumIS0.02.csv"))